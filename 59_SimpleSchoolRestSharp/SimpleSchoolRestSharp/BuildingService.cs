...
namespace SimpleSchoolRestSharp
{
    class BuildingService
    {
        ...
        public void GetRoomsByBuilding()
        {
            Console.Write("Enter building id: ");
            int id = int.Parse(Console.ReadLine());

            var request = new RestRequest($"api/buildings/{id}/rooms", Method.GET);
            var response = _client.Get<List<Room>>(request);

            if (response.IsSuccessful)
            {
                Console.WriteLine($"Buidling {id}");
